...
void ProcessRequest(RecommenderRequest &recommender_request,
                    uint64_t unique_request_id_value, int tid);

// Global variable declarations.
/* dataset_dim is global so that we can validate query dimensions whenever
   batches of queries are received.*/

unsigned int recommender_parallelism = 0, number_of_response_threads = 0,
             number_of_cf_servers = 1, dispatch_parallelism = 0;
...
CompletionQueue *cf_srv_cq = NULL;
...
private:
  ...
  public:
    ...
    // with the gRPC runtime.
    CallData(RecommenderService::AsyncService *service,
    ...
    void Proceed(int tid) {
      if (status_ == CREATE) {
        ...
        service_->RequestRecommender(&ctx_, &recommender_request_, &responder_,
                                     cq_, cq_, this);
        ...
        // num_reqs->AtomicallyIncrementCount();
        // The actual processing.
        ProcessRequest(recommender_request_, unique_request_id_value, tid);
...
public:
  ...
  /* Assambles the client's payload, sends it and presents the response back
     from the server.*/
  void GetRating(const uint32_t cf_server_id, const bool util_present,
                 uint64_t request_id, CFRequest request_to_cf_srv) {
    // Create RCP request by adding queries, point IDs, and number of NN.
    CreateCFServiceRequest(cf_server_id, util_present, &request_to_cf_srv);
    ...
    call->response_reader =
        stub_->AsyncCF(&call->context, request_to_cf_srv, cf_srv_cq);
  ...
  void AsyncCompleteRpc() {
    ...
    if (call->status.ok()) {
      ...
      /* Create local DistCalc, CFSrvTimingInfo, CFSrvUtil variables,
         so that this thread can unpack received cf_srv data into these
         variables and then grab a lock to append to the response array in the
         map.*/
      uint64_t start_time = GetTimeInMicro();
      float rating = 0.0;
      CFSrvTimingInfo cf_srv_timing_info;
      CFSrvUtil cf_srv_util;
      UnpackCFServiceResponse(call->reply, &rating, &cf_srv_timing_info,
                              &cf_srv_util);
      ...
      try {
        ...
        CHECK(false,
              "ERROR: Map entry corresponding to request id does not exist\n");
      }
      map_coarse_mutex.unlock();

      map_fine_mutex[unique_request_id]->lock();
      auto &map_el =response_count_down_map[unique_request_id]; 
      int cf_srv_resp_id =
          map_el.responses_recvd;
      *(response_count_down_map[unique_request_id]
            .response_data[cf_srv_resp_id]
            .rating) = rating;
      *(response_count_down_map[unique_request_id]
            .response_data[cf_srv_resp_id]
            .cf_srv_timing_info) = cf_srv_timing_info;
      *(response_count_down_map[unique_request_id]
            .response_data[cf_srv_resp_id]
            .cf_srv_util) = cf_srv_util;
      response_count_down_map[unique_request_id]
          .response_data[cf_srv_resp_id]
          .cf_srv_timing_info->unpack_cf_srv_resp_time = end_time - start_time;

      uint64_t total_time = response_count_down_map[unique_request_id]
                                .recommender_reply->total_calc_time();
      response_count_down_map[unique_request_id]
          .recommender_reply->set_total_calc_time(
              cf_srv_timing_info.cf_srv_time + total_time);

      if (response_count_down_map[unique_request_id].responses_recvd !=
          (number_of_cf_servers - 1)) {
        response_count_down_map[unique_request_id].responses_recvd++;
        map_fine_mutex[unique_request_id]->unlock();

      } else {
        uint64_t cf_srv_resp_start_time =
            response_count_down_map[unique_request_id]
                .recommender_reply->get_cf_srv_responses_time();
        response_count_down_map[unique_request_id]
            .recommender_reply->set_get_cf_srv_responses_time(
                GetTimeInMicro() - cf_srv_resp_start_time);
        ...
        MergeAndPack(
            response_count_down_map[unique_request_id].response_data,
            number_of_cf_servers,
            response_count_down_map[unique_request_id].recommender_reply);

        end_time = GetTimeInMicro();
        // response_count_down_map[unique_request_id].recommender_reply->set_recommender_time(end_time
        // - start_time);
        response_count_down_map[unique_request_id]
            .recommender_reply->set_pack_recommender_resp_time(end_time -
                                                               start_time);
        // uint64_t recommender_time =
        // response_count_down_map[unique_request_id].recommender_reply->recommender_time();
        // uint64_t total_time =
        // response_count_down_map[unique_request_id].recommender_reply->total_calc_time();
        // response_count_down_map[unique_request_id].recommender_reply->set_total_calc_time(total_time
        // + (GetTimeInMicro() - recommender_time));
        // response_count_down_map[unique_request_id].recommender_reply->set_recommender_time(GetTimeInMicro()
        // - recommender_time);
        ...
        uint64_t prev_rec = response_count_down_map[unique_request_id]
                                .recommender_reply->recommender_time();
        response_count_down_map[unique_request_id]
            .recommender_reply->set_recommender_time(prev_rec +
                                                     (GetTimeInMicro() - s1));
        map_fine_mutex[unique_request_id]->unlock();
...
void ProcessRequest(RecommenderRequest &recommender_request,
  ...
  map_coarse_mutex.unlock();
  auto &map_el = response_count_down_map[unique_request_id_value];
  ...
  map_el.responses_recvd = 0;
  map_el.response_data.resize(
      number_of_cf_servers, ResponseData());
  map_el
      .recommender_reply->set_request_id(recommender_request.request_id());
  map_el
      .recommender_reply->set_num_inline(recommender_parallelism);
  map_el
      .recommender_reply->set_num_workers(dispatch_parallelism);
  map_el
      .recommender_reply->set_num_resp(number_of_response_threads);
  ...
  /* If the load generator is asking for util info,
     ...
     the recommender must read /proc/stat to provide user, system, and io
     times.*/
  if (util_present) {
    uint64_t start = GetTimeInMicro();
    uint64_t user_time = 0, system_time = 0, io_time = 0, idle_time = 0;
    GetCpuTimes(&user_time, &system_time, &io_time, &idle_time);
    // map_fine_mutex[unique_request_id_value]->lock();
    map_el
        .recommender_reply->mutable_util_response()
        ->mutable_recommender_util()
        ->set_user_time(user_time);
    map_el
        .recommender_reply->mutable_util_response()
        ->mutable_recommender_util()
    ...
    map_el
        .recommender_reply->mutable_util_response()
        ->set_util_present(true);
    map_el
        .recommender_reply->set_update_recommender_util_time(GetTimeInMicro() -
                                                             start);
    // map_fine_mutex[unique_request_id_value]->unlock();
  }
  uint64_t start_time = GetTimeInMicro();
  ...
  int user = 0, item = 0;
  UnpackRecommenderServiceRequest(recommender_request, &user, &item,
                                  &request_to_cf_srv);
#ifndef NODEBUG
  std::cout << "aft unpack\n";
#endif
  ...
  // map_fine_mutex[unique_request_id_value]->lock();
  map_el
      .recommender_reply->set_unpack_recommender_req_time(
          (end_time - start_time));
  // map_el.recommender_reply->set_recommender_time(GetTimeInMicro());
  // map_fine_mutex[unique_request_id_value]->unlock();
  // float points_sent_percent = PercentDataSent(point_ids, queries_size,
  // dataset_size); printf("Amount of dataset sent to cf_srv server in the form
  // of point IDs = %.5f\n", points_sent_percent);
  //(*response_count_down_map)[unique_request_id_value]->recommender_reply->set_percent_data_sent(points_sent_percent);

  // map_fine_mutex[unique_request_id_value]->lock();
  map_el
      .recommender_reply->set_get_cf_srv_responses_time(GetTimeInMicro());
  // map_fine_mutex[unique_request_id_value]->unlock();

  for (unsigned int i = 0; i < number_of_cf_servers; i++) {
    int index = (tid * number_of_cf_servers) + i;
    cf_srv_connections[index]->GetRating(
        i, util_present, unique_request_id_value, request_to_cf_srv);
  }
  e1 = GetTimeInMicro() - s1;
  map_el
      .recommender_reply->set_recommender_time(e1);
  map_fine_mutex[unique_request_id_value]->unlock();
}
...
void Perf() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::cout << "cs\n";
      std::string s =
          "sudo perf stat -e cs -I 30000 -p " + std::to_string(getpid());
...
void SysCount() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::cout << "syscnt\n";
      std::string s = "sudo /usr/share/bcc/tools/syscount -i 30 -p " +
                      std::to_string(getpid()) + " > " + "syscount.txt";
      char *cmd = new char[s.length() + 1];
      std::strcpy(cmd, s.c_str());
      ExecuteShellCommand(cmd);
...
void Hardirqs() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::cout << "hardirqs\n";
      std::string s =
          "sudo /usr/share/bcc/tools/hardirqs -d -T 30 1 > hardirqs.txt";
      char *cmd = new char[s.length() + 1];
      std::strcpy(cmd, s.c_str());
      ExecuteShellCommand(cmd);
      break;
...
void Wakeuptime() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::string s = "sudo /usr/share/bcc/tools/wakeuptime -p " +
                      std::to_string(getpid()) + " 30 > wakeuptime.txt";
      char *cmd = new char[s.length() + 1];
      std::strcpy(cmd, s.c_str());
      ExecuteShellCommand(cmd);
      break;
...
void Softirqs() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::string s =
          "sudo /usr/share/bcc/tools/softirqs -T 30 1 -d > softirqs.txt";
      char *cmd = new char[s.length() + 1];
      std::strcpy(cmd, s.c_str());
      ExecuteShellCommand(cmd);
      break;
...
void Tcpretrans() {
  while (true) {
    if (started->AtomicallyReadFlag()) {
      std::string s =
          "sudo /usr/share/bcc/tools/tcpretrans -c -l > tcpretrans.txt";
      char *cmd = new char[s.length() + 1];
      std::strcpy(cmd, s.c_str());
      ExecuteShellCommand(cmd);
      break;
...
int main(int argc, char **argv) {
  ...
  if (argc == 7) {
    ...
    CHECK(false, "<./recommender_server> <number of cf servers> <cf server ips "
                 "file> <ip:port number> <recommender parallelism>\n");
  }
  ...
  for (unsigned int i = 0; i < dispatch_parallelism; i++) {
    for (unsigned int j = 0; j < number_of_cf_servers; j++) {
      ...
      cf_srv_connections.emplace_back(new CFServiceClient(
